...
os.chdir('/Users/reach/Desktop/Git/CINM/')
...
if parquet_inputs:
    ...
    for inx, file_id in enumerate(files):
        data[sheet_names[inx]] = pd.read_parquet(os.path.join(parquet_path_data, file_id),
                                                 engine='pyarrow')  # read them
else:
...
for sheet_name in sheets:
    ...
    data[sheet_name]['Overall'] = ' Overall'
    data[sheet_name]['macroregion2'] = data[sheet_name].apply(lambda row: 'Kyiv' if row['oblast'] == 'UA80' else row['macroregion'], axis=1)
...
if daf_numeric.shape[0] > 0:
    for i, daf_row in daf_numeric.iterrows():
        ...
        if res == False:
            raise ValueError(
                f"Variable {daf_row['variable']} from datasheet {daf_row['datasheet']} is not numeric, but you want to apply a mean function to it in your DAF")
...
disaggregations_full = disaggregation_creator(daf_final, data, filter_dict, tool_choices, tool_survey,
                                              label_colname=label_colname, check_significance=sign_check,
                                              weight_column=weighting_column)


df_with_id_oblast = daf_final[daf_final["admin"] == "oblast"]["ID"].to_list()
df_with_id_raion = daf_final[daf_final["admin"] == "raion"]["ID"].to_list()
df_with_id_strata = daf_final[daf_final["admin"] == "strata_name"]["ID"].to_list()

oblat2macro = pd.read_excel("resources/macro_oblast_raion.xlsx", sheet_name="oblast")
raion2oblast = pd.read_excel("resources/macro_oblast_raion.xlsx", sheet_name="raion")
strata2oblast = pd.read_excel("resources/macro_oblast_raion.xlsx", sheet_name="strata_name")

for i in range(len(disaggregations_full)):
    if disaggregations_full[i][1] in df_with_id_oblast:
        frame = oblat2macro.merge(disaggregations_full[i][0], how="left", left_on="oblast", right_on="admin_category")
        frame = frame.dropna(subset=['admin_category'])
        frame = frame.drop('oblast', axis=1)
        id_old = disaggregations_full[i][1]
        label_old = disaggregations_full[i][2]
        sign_old = disaggregations_full[i][3]
        disaggregations_full[i] = (frame, id_old, label_old, sign_old)
    if disaggregations_full[i][1] in df_with_id_raion:
        frame = raion2oblast.merge(disaggregations_full[i][0], how="left", left_on="raion", right_on="admin_category")
        frame = frame.dropna(subset=['admin_category'])
        frame = frame.drop('raion', axis=1)
        id_old = disaggregations_full[i][1]
        label_old = disaggregations_full[i][2]
        sign_old = disaggregations_full[i][3]
        disaggregations_full[i] = (frame, id_old, label_old, sign_old)
    if disaggregations_full[i][1] in df_with_id_strata:
        frame = strata2oblast.merge(disaggregations_full[i][0], how="left", left_on="strata_name", right_on="admin_category")
        frame = frame.dropna(subset=['admin_category'])
        frame = frame.drop('strata_name', axis=1)
        id_old = disaggregations_full[i][1]
        label_old = disaggregations_full[i][2]
        sign_old = disaggregations_full[i][3]
        disaggregations_full[i] = (frame, id_old, label_old, sign_old)
...
#     concatenated_df_orig = concatenated_df_orig[
#         (concatenated_df_orig['admin'] != 'Total') & (concatenated_df_orig['disaggregations_category_1'] != 'Total')]
# else:
#     concatenated_df_orig = concatenated_df_orig[(concatenated_df_orig['admin'] != 'Total')]
#
# disagg_columns_og = [col for col in concatenated_df_orig.columns if
#                      col.startswith('disaggregations') and not col.endswith('orig')]
...
#         concatenated_df_orig[column + '_orig'] = concatenated_df_orig[column + '_orig'].fillna(
#             concatenated_df_orig[column])
#
...
if 'disaggregations_category_1' in concatenated_df.columns:
    concatenated_df = concatenated_df[
        (concatenated_df['admin'] != 'Total') & (concatenated_df['disaggregations_category_1'] != 'Total')]
else:
...
for data_frame in [disaggregations_perc_new, disaggregations_count_new, disaggregations_count_w_new]:
    ...
    if pd.notna(daf_final['join']).any():
        ...
        if any(child_rows['ID'].isin(child_rows['join'])):
            raise ValueError(
                'Some of the join tables are related to eachother outside of their relationship with the parent row. Please fix this')
        ...
        for index, child_row in child_rows.iterrows():
            ...
            if child_index not in daf_final['ID'].values:
                raise ValueError(
                    f'The specified parent index in join column for child row ID = {child_index} doesnt exist in the DAF file')
            
            parent_row = daf_final[daf_final['ID'].isin(child_row[['join']])]
            parent_index = parent_row.iloc[0]['ID']
            
            # check that the rows are idential
            parent_check = parent_row[['disaggregations', 'func', 'calculation', 'admin', 'q.type']].reset_index(
                drop=True)
            child_check = child_row.to_frame().transpose()[
                ['disaggregations', 'func', 'calculation', 'admin', 'q.type']].reset_index(drop=True)
            ...
            if not check_result:
                raise ValueError(
                    f"Joined rows (parent: {str(parent_row['ID'].values)} and child: {str(child_row['ID'])}) are not identical in terms of admin, calculations, function and disaggregations")
            ...
            columns_to_merge = [item for item in parent_tupple_data.columns if
                                'disaggregations' in item or 'admin' in item]
